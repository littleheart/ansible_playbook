...
body_text=/tmp/mailbody_memory.txt
...
then
    echo "server name: $(hostname) - Memory usage in Intermidiate state - $memory_precentage" > $body_text
    sendemail -f "$from_address" -u "$subject1" -t "$to_address" -s "$gmail_gateway" -o tls=yes -xu "$username" -xp "$password" -o message-file="$body_text" -q
elif [ $memory -gt 75 ];
then
    echo "server name: $(hostname) - Memory usage in Intermidiate state - $memory_precentage" > $body_text
    sendemail -f "$from_address" -u "$subject1" -t "$to_address" -s "$gmail_gateway" -o tls=yes -xu "$username" -xp "$password" -o message-file="$body_text" -q

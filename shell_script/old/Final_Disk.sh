...
do
  ...
  if [ $used -ge 10 ]; then
      ...
      mailx -v -r "$from_address" -s "$subject1" -S smtp="$gmail_gateway" -S smtp-use-starttls -S smtp-auth=login -S smtp-auth-user="$username" -S smtp-auth-password="$password" -S ssl-verify=ignore $to_address
